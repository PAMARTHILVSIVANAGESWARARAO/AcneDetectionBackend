...
- **severe:** ⚠️ Dermatologist supervision mandatory
...
**Progression:**
- Day 1 → 7: Initial phase, introducing treatments
...
✅ OTP Validation: 6-digit codes, 5-minute expiry
✅ Email Verification: Required before login
✅ CORS Protection: Only whitelisted origins allowed
✅ Rate Limiting: Prevents brute force attacks
✅ SQL/NoSQL Injection: Query parameterization
...
4️⃣ DATA FLOW WITH EXTERNAL SERVICES
   ┌─────────────────────────────────┐
   │      User's React Vite App      │
   │  (localhost:5173)               │
   └────────────┬────────────────────┘
                │ HTTPS/CORS Allowed
   ┌────────────▼───────────────────────┐
   │    AcneAI Backend (Express.js)      │
   │    (localhost:5000)                 │
   └────────────┬──────────┬─────────────┘
                │          │
    ┌───────────▼──┐   ┌───▼──────────────┐
...
5️⃣ STATUS FLOW
   START → Registered → Verified → Questionnaire → Images → 
   Day 1 Created → Day 1 Reviewed → Day 2 Created → ... → 
